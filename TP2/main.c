...
int main()
{
    ...
    opcion =common_getMenu("1.ALTA\n2.MODIFICAR\n3.BAJA\n4.INFORMAR\n5.EXIT\n","Opcion invalida. Reingrese",1,5,3);
        do
        {
            common_validarOpcionesMenu(&opcion,hasValueOpc1,"First must add a new employee \n");
            switch(opcion)
                {
                case 1:
                    hasValueOpc1 = TRUE;
                    if(!common_isValidReturnedFunctionValue(addEmployee(listado,LENGTH,++id)))
                    {
                    ...
                    break;
                case 2:
                    if(!common_isValidReturnedFunctionValue(modifyEmployee(listado,LENGTH)))
                    {
                        printf("Terminal error. System will exit \n");
                        exit = TRUE;
                    }
                    break;
                case 3:
                    if(!common_isValidReturnedFunctionValue(removeEmployee(listado,LENGTH)))
                    {
                        printf("Terminal error. System will exit \n");
                        exit = TRUE;
                    }
                ...
                case 4:
                    ...
                    {
                        if(common_isValidReturnedFunctionValue(sortEmployees(listado,LENGTH,common_getMenu("Enter the order to list the employees:\n0. Down\n1. UP\n","Opcion invalida. Reingrese",0,1,2))))
                        {
                    ...
                    else
                    {
                        ...
                        if(common_isValidReturnedFunctionValue((int) total))
                        {
                        ...
                        if(common_isValidReturnedFunctionValue((int) averageSalary))
                        {
                        ...
                        if(common_isValidReturnedFunctionValue(cantOfSalaryUpAvg))
                        {
